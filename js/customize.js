...
function buildSkinRow() {
  ...
  FaceParts.SKIN_COLORS.forEach(c => {
    const dot = _colorDot(c.hex, c.id === state.skinColor, () => {
...
function buildFaceGrids() {
  ...
  // Eye color
  document.getElementById('sub-eyes').appendChild(
    _colorSection('눈 색', FaceParts.EYE_COLORS, 'eyeColor')
  );
}
...
function buildHairGrid() {
  ...
  document.getElementById('panel-hair').appendChild(
    _colorSection('머리색', FaceParts.HAIR_COLORS, 'hairColor')
  );
}
...
function buildClothingPanel() {
  ...
  intro.style.cssText = 'font-size:12px;color:#aaa;margin-bottom:14px;';
  ...
  FaceParts.CLOTHING_COLORS.forEach(c => {
    ...
    circle.style.cssText = `width:44px;height:44px;border-radius:50%;background:${c.hex};
      border:3px solid ${c.id===state.clothingColor?'#007AFF':'transparent'};
      box-shadow:inset 0 0 0 1.5px rgba(0,0,0,0.1),0 1px 4px rgba(0,0,0,0.12);
    ...
    item.appendChild(circle); item.appendChild(lbl);
...
function buildPersonalityPanel() {
  ...
  const intro = document.createElement('div');
  intro.className = 'pers-intro'; intro.textContent = '직관적으로 느껴지는 대로 선택하세요.';
  ...
  QUESTIONS.forEach((q, idx) => {
    ...
    const leftLbl  = document.createElement('div'); leftLbl.className = 'scale-label';      leftLbl.textContent  = q.left;
    const rightLbl = document.createElement('div'); rightLbl.className = 'scale-label right'; rightLbl.textContent = q.right;
    ...
    row.append(leftLbl, boxes, rightLbl);
...
function buildGrid(containerId, partType, types, stateKey) {
  ...
  types.forEach(t => {
    ...
    FaceParts.drawIcon(iconCanvas, partType, t.id, state); // async, updates in background
...
function _colorSection(label, colors, stateKey) {
  ...
  colors.forEach(c => {
    const dot = _colorDot(c.hex, c.id===state[stateKey], () => {
      state[stateKey] = c.id;
...
function _colorDot(hex, selected, onClick) {
  const dot = document.createElement('div');
  dot.className = 'color-dot' + (selected?' selected':'');
...
function refreshIcons() {
  document.querySelectorAll('.part-item').forEach(item => {
    ...
    const partType = {'eyes-grid':'eyes','nose-grid':'nose','mouth-grid':'mouth','hair-grid':'hair'}[gridId];
...
function computeMBTI() {
  ...
  QUESTIONS.forEach((q, i) => { score[q.dim] += (state.personality[`q${i}`]||3.5) - 3.5; });
